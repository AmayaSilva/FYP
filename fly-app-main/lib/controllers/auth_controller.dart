...
class AuthController {
  ...
  Future<bool> checkUser(String userId) async {
    ...
    final doc =
        await FirebaseFirestore.instance.collection("atcos").doc(userId).get();
    if (doc.exists && doc.data() != null) {
      User_two pharmacyModel = User_two.fromJson(doc.data()!);
      await _saveToStatic(pharmacyModel);
      return true;
    } else {
      return false;
    }
  ...
  Future<String> _uploadImage(XFile picked) async {
    final ref = FirebaseStorage.instance.ref().child(
        'pharmacy/${UserData.atcid}/${DateTime.now().millisecondsSinceEpoch}/${picked.path.split('/').last}.jpg');
    final bytes = await picked.readAsBytes();
    final uploadTask = await ref.putData(bytes);
    return await uploadTask.ref.getDownloadURL();
  }

  Future<User_two?> getPharmacy() async {
